...
public class HalmaBoard extends Observable implements Board 
{
	...
	// * fungsi untuk jika game ad yg sudah menang . jika benar , currentPlayer is victorious.
	...
	public boolean isMoveValid(Move move, Player player)
	{
		...
		// check multi jump
		startField = field[halmaMove.getFromXPosition()][halmaMove.getFromYPosition()];
		Field targetField = field[halmaMove.getToXPosition()][halmaMove.getToYPosition()];
	...
	private boolean checkSingleJump(HalmaMove move) 
	{
		...
		Field startField = field[move.getFromXPosition()][move.getFromYPosition()];
		...
		for (int i = 0; i < occFields.size(); i++) 
		{
			...
			for (int j = 0; j < frees.size(); j++)
			{
				...
				/*
				 Target found? check direction, if not valid, whole validationreturns false
				 */
				if (free.getXPosition() == targetField.getXPosition() && free.getYPosition() == targetField.getYPosition())
				{
	...
	/*
	  Searches for multi jump conditions (jump over more than one occupied fields
	  return is move a multi jump which is valid according to Halma rules
	*/
	private boolean checkMultiJump(int x, int y, Field targetField, int previousX, int previousY) 
	{
		...
		if (y < this.getWidth() - 2 && !(y + 2 == previousY) && !getFieldState(field[x][y + 2]).equals("invalid"))

			// ignore startfield
			if (!(startField.getYPosition() == y + 2 && startField.getXPosition() == x))
				...
				if (!getFieldState(field[x][y + 2]).equals(status)) 
				{
					...
					if (getFieldState(field[x][y + 2]).equals("free")) 
					{
						...
						{
							if ((isDirectionValid(getLastFreeField(path),field[x][y + 2]))) 
							{
								path.add(field2);
								if (checkMultiJump(x, y + 2, targetField, x, y)) return true;
							}
						}
		...
		if (y > 2 && !(y - 2 == previousY) && !(getFieldState(field[x][y - 2]).equals("invalid")))
			// ignore startfield
			if (!(startField.getYPosition() == y - 2 && startField.getXPosition() == x))
				// Alternating field state?
				if (!getFieldState(field[x][y - 2]).equals(status) && !getFieldState(field[x][y - 2]).equals("invalid")) 
				{
					Field field2 = field[x][y];
					...
					if (getFieldState(field[x][y - 2]).equals("free")) 
					{
						...
						// no last free field? then it's the beginning of a multi jump
						else if (isDirectionValid(startField, field[x][y - 2])) 
						{
							path.add(field2);
							if (checkMultiJump(x, y - 2, targetField, x, y))
		...
		if (x < this.getHeight() - 2 && x > 0 && y > 0 && y < this.getWidth() - 1 && (!(x - 2 == previousX) || !(y + 1 == previousY)))
		
			// ignore startfield
			
			if (!(startField.getYPosition() == y + 1 && startField.getXPosition() == x - 2))
			
				// Alternating field state?
				
				if (!getFieldState(field[x - 2][y + 1]).equals(status) && !getFieldState(field[x - 2][y + 1]).equals("invalid")) 
				{
					Field field2 = field[x][y];
					// Valid Direction? Validation just works with free fields
					if (getFieldState(field[x - 2][y + 1]).equals("free")) 
					{
						if (path.size() > 0 && getLastFreeField(path) != null) 
						{
							if ((isDirectionValid(getLastFreeField(path),field[x - 2][y + 1])))
							{
								path.add(field2);
						...
						else if ((isDirectionValid(startField,field[x - 2][y + 1]))) 
						{
							path.add(field2);
							if (checkMultiJump(x - 2, y + 1, targetField, x, y)) 
							{
		...
		if (x < this.getHeight() - 2 && x > 0 && y < this.getWidth() - 1 && y > 0 && (!(x + 2 == previousX) || !(y - 1 == previousY)))
			// ignore startfield
			if (!(startField.getYPosition() == y - 1 && startField.getXPosition() == x + 2))
			...
			if (!(startField.getYPosition() == y + 1 && startField.getXPosition() == x + 2))
				...
				if (!getFieldState(field[x + 2][y + 1]).equals(status)&& !getFieldState(field[x + 2][y + 1]).equals("invalid")) 
				{
					...
					if (getFieldState(field[x + 2][y + 1]).equals("free")) 
					{
						...
						}// no last free field? then it's a beginning of a multi jump
						else if (isDirectionValid(startField, field[x + 2][y + 1]))
						{
							path.add(field2);
							if (checkMultiJump(x + 2, y + 1, targetField, x, y))
		...
		path.add(new Field(SEPARATOR, SEPARATOR, SEPARATOR, SEPARATOR,SEPARATOR, false));
		return false;
	}
	...
	/*
	 Searches for twofold jump sequences return true if jump path contains the same sequence of fields more than once
	 */
		...
		for (int i = 0; i < subPaths.size(); i++)
		{
			...
			if (sub.size() == sample.size()) 
			{
				for (int j = 0; j < sub.size(); j++) 
				{
					...
					{
						if (sub.get(i).getXPosition() != sample.get(k).getXPosition()) 
						{
							return false;
						}
					}
	...
	*/
	private boolean isFieldVisited(ArrayList<Field> fieldList, Field field) 
	{
		for (int i = 0; i < fieldList.size(); i++) 
		{
			...
			if (oldField.getXPosition() == field.getXPosition() && oldField.getYPosition() == field.getYPosition()) 
			{
				return true;
			}
		}
			...
			for (int j = y - 2; j <= y + 2; j++) 
			{
				...
				if (i >= 0 && j >= 0 && i < (fields.length) && j < (fields[i].length)) 
				{
					...
					if (arrField.getFieldType() != HalmaSettings.NO_FIELD) 
					{
						...
						// neighborhood
						if ((arrField.getXPosition() != field.getXPosition())|| (arrField.getYPosition() != field.getYPosition())) 
						{
				...
				{
					// create dummy fields (Fields out of board range)
	...
	/*
	  ...
	  move has to be on a line without changes of direction whithin one singlemove.
	  it validates just the relation between a target field and the last freefield
	  param startfield, targetfield pairs of start field and target field
